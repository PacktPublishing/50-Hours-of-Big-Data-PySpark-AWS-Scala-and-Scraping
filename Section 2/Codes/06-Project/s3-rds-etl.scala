...
object GlueApp {
  ...
  def main(sysArgs: Array[String]) {
        ...
        df.show()
        df.write.format("jdbc").option("url", "jdbc:postgresql://database-1.cs8ljxah3rgm.us-east-1.rds.amazonaws.com:5432/").option("dbtable", "etlDataLoad").option("user", "postgres").option("password", "qwerasdfzxcv123").save()     
